...
// We need to replace `isAdmin()` with `isWriteAdmin()` ONLY on lines containing `allow write`, `allow create`, `allow update`, `allow delete` where it doesn't contain `read`, `get`, `list`.
// Specifically:
...
// Manually fix mixed lines like `allow read, write: if isAdmin();`
for (let i = 0; i < lines.length; i++) {
