...
import { Shield, Zap, Cpu, ArrowRight, ShieldCheck, Fingerprint, Lock, Globe, Command, Sparkles, Box, CreditCard, ChevronRight, Activity } from 'lucide-react';
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#000] text-zinc-400 font-sans selection:bg-[#ff5f00] selection:text-white overflow-x-hidden relative">
      ...
      {/* Stats Section */}
      <section className="py-16 lg:py-32 border-y border-white/5 bg-[#030303]/50 backdrop-blur-3xl relative z-10">
         <div className="max-w-[1920px] mx-auto px-4 lg:px-16">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-32">
                {[
                    { label: 'Giao dịch an toàn', value: '45M+', suffix: 'Hàng ngày' },
                    { label: 'Thời gian phản hồi', value: '0.04', suffix: 'ms' },
                    { label: 'Quốc gia hiện diện', value: '180+', suffix: 'Toàn cầu' },
                    { label: 'Uptime hệ thống', value: '99.9', suffix: '%' },
                ].map((stat, i) => (
                    <motion.div 
                      key={i} 
                    ...
                    >
                        <div className="flex items-baseline lg:gap-2">
                           <span className="text-3xl md:text-7xl font-serif italic text-white tracking-tighter group-hover:text-[#f79e1b] transition-colors">{stat.value}</span>
                           <span className="text-xs lg:text-xl font-bold text-zinc-700 uppercase tracking-widest ml-1">{stat.suffix}</span>
                        </div>
                        <p className="text-[8px] lg:text-[11px] font-black text-zinc-500 uppercase tracking-[0.3em] lg:tracking-[0.4em] pt-1">{stat.label}</p>
                    </motion.div>
                ))}
            </div>
         </div>
      ...
      <section id="features" className="py-32 lg:py-64 px-4 lg:px-16 relative">
         <div className="max-w-[1920px] mx-auto space-y-20 lg:space-y-32">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-10">
               {[
                  { icon: ShieldCheck, title: "Identity Guard", desc: "Mã hóa sinh trắc học và quản lý định danh số chuẩn Zero-Knowledge.", color: "#eb001b" },
                  { icon: Zap, title: "AI Neural Hub", desc: "Xử lý dữ liệu, trích xuất văn bản và tự động hóa tác vụ bởi Gemini 3.0 Flash.", color: "#f79e1b" },
                  { icon: Globe, title: "Global Sync", desc: "Đồng bộ hóa dữ liệu cá nhân xuyên quốc gia với độ trễ tối thiểu.", color: "#323c46" },
                  { icon: Lock, title: "Private Vault", desc: "Két sắt kỹ thuật số bảo mật chuẩn quân sự AES-256 GCM.", color: "#323c46" },
                  { icon: Activity, title: "Real-time Monitor", desc: "Theo dõi mọi hoạt động đăng nhập và vị trí thiết bị theo thời gian thực.", color: "#323c46" },
                  { icon: Cpu, title: "Cloud Engine", desc: "Hệ thống hạ tầng phân tán mạnh mẽ, hỗ trợ mọi quy mô người dùng.", color: "#323c46" }
               ].map((f, i) => (
                  <motion.div 
                     key={i}
                     whileHover={{ y: -8, scale: 1.01 }}
                     className="group p-8 lg:p-12 bg-zinc-950 border border-white/5 rounded-[2rem] lg:rounded-[3rem] hover:border-[#f79e1b]/30 transition-all duration-700 relative overflow-hidden"
                  >
      ...
      <footer className="max-w-[1920px] mx-auto px-4 lg:px-16 py-20 lg:py-32 border-t border-white/5 relative z-10">
         <div className="grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-24">
            ...
            <div className="md:col-span-2 space-y-6 lg:space-y-12">
               <h4 className="text-[10px] lg:text-[12px] font-black text-white uppercase tracking-[0.4em] lg:tracking-[0.5em]">Tài nguyên</h4>
               <div className="flex flex-col gap-4 lg:gap-6">
                  <NavItem href="/utilities">Công cụ</NavItem>
                  <NavItem href="/releases">Phiên bản</NavItem>
                  <NavItem href="/terms">Điều khoản</NavItem>
               </div>
            </div>

            <div className="md:col-span-2 space-y-6 lg:space-y-12">
               <h4 className="text-[10px] lg:text-[12px] font-black text-white uppercase tracking-[0.4em] lg:tracking-[0.5em]">Hỗ trợ</h4>
               <div className="flex flex-col gap-4 lg:gap-6">
                  <NavItem href="/contact">Liên hệ</NavItem>
                  <NavItem href="/privacy">Bảo mật</NavItem>
                  <NavItem href="/about">Giới thiệu</NavItem>
               </div>
            </div>

            <div className="md:col-span-4 space-y-6 lg:space-y-12">
               <h4 className="text-[10px] lg:text-[12px] font-black text-white uppercase tracking-[0.4em] lg:tracking-[0.5em]">Đăng ký nhận tin</h4>
               <div className="relative">
                  <input type="text" placeholder="Địa chỉ email..." className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 lg:px-8 py-5 lg:py-6 text-white text-base lg:text-lg focus:outline-none focus:border-[#f79e1b] transition-all" />
                  <button className="absolute right-2 top-2 bottom-2 px-4 lg:px-8 bg-white text-black rounded-xl font-black uppercase text-[9px] lg:text-[10px] tracking-widest hover:bg-[#f79e1b] hover:text-white transition-all">Submit</button>
               </div>
               <p className="text-[10px] lg:text-[11px] text-zinc-700 italic">Bằng cách đăng ký, bạn đồng ý với Chính sách Bảo mật.</p>
            </div>
         </div>
